...
class Bag {
  ...
  public function makeAbsolute($path)
  {
    ...
    array_unshift($components, $rootComponents);
    return implode(DIRECTORY_SEPARATOR, $rootComponents);
  }
  ...
  /**
   * Write out the parts of a new BagIt bag.
   */
    ...
    if (!file_exists($fullPath)) {
      ...
      if (count($lines) !== 2) {
        $this->loadErrors[] = [
          'file' => 'bagit.txt',
          'message' => sprintf(
            "File should contain exactly 2 lines found %b", count($lines)
          ),
        ];
      } else {
        if (!preg_match("~^BagIt\-Version: (\d+)\.(\d+)$~", $lines[0],
          $match)) {
          $this->loadErrors[] = [
            'file' => 'bagit.txt',
        ...
        if (!preg_match("~^Tag\-File\-Character\-Encoding: (.*)$~",
          $lines[1], $match)) {
          $this->loadErrors[] = [
            'file' => 'bagit.txt',
  ...
  private function updateBagIt()
  {
    ...
    $output = sprintf("BagIt-Version: %d.%d\n" .
      "Tag-File-Character-Encoding: %s\n", $version['major'], $version['minor'],
      $this->getFileEncoding());

    file_put_contents($this->makeAbsolute("bagit.txt"),
      $output);
  }
  ...
  {
    $fullPath = $this->makeAbsolute($path);
    if (file_exists($fullPath)) {
      if (is_file($fullPath)) {
        unlink($fullPath);
        $this->checkForEmptyDir($path);
